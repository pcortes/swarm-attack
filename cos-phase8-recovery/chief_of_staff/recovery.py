...
- RetryStrategy enum for retry/escalation decisions
- ErrorCategory enum for error classification
- RecoveryLevel enum for retry/escalation decisions (legacy)
...
class RetryStrategy(Enum):
    ...
    Defines the available retry strategies that drive recovery behavior:
    - SAME: Retry the same approach (for transient errors)
    - ALTERNATIVE: Try a different approach (extension point for future work)
    - CLARIFY: Request manual clarification via checkpoint
...
class ErrorCategory(Enum):
    ...
    - TRANSIENT: Temporary errors like network issues, timeouts, rate limits -> Level 1 retry
    - SYSTEMATIC: Approach-related errors like wrong methodology -> falls through to Level 4
    - FATAL: Unrecoverable errors like security issues -> immediate Level 4 escalation
    """
        ...
        checkpoint = Checkpoint(
            ...
            options=[
                CheckpointOption(
                    ...
                    action="skip",
                    description="Mark goal as skipped and continue with next goal",
                ),
                CheckpointOption(
                    label="Retry with modifications",
                    action="retry_modified",
                    description="Retry the goal with human-provided modifications",
                ),
                CheckpointOption(
                    label="Manual intervention",
                    action="manual",
                    description="Pause autopilot for manual intervention",
                ),
            ],
