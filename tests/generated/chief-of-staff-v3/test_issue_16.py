...
class TestTestCriticEvaluate:
    ...
    def test_evaluate_unsupported_focus_returns_error(self):
        """Evaluate with unsupported focus should return error score."""
        mock_llm = Mock()
        critic = TestCritic(focus=CriticFocus.STYLE, llm=mock_llm)
        result = critic.evaluate("def test_foo(): pass")
        assert result.score == 0.0
        assert result.approved is False
        assert "Unsupported focus" in result.issues[0]
...
class TestTestCriticTruncation:
    ...
    def test_max_test_chars_is_4000(self):
        ...
        assert TestCritic.MAX_TEST_CHARS == 4000
...
class TestTestCriticLLMParsing:
    ...
    def test_critic_name_includes_focus(self):
        ...
        assert "COVERAGE" in result.critic_name
    ...
    def test_edge_cases_critic_name(self):
        ...
        assert "TestCritic" in result.critic_name
        assert "EDGE_CASES" in result.critic_name
